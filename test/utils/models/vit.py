...
# here we list all keys to be renamed (original name on the left, our
# name on the right)
def create_rename_keys(config, base_model=False):
    rename_keys = []
    for i in range(config.num_hidden_layers):
        # encoder layers: output projection, 2 feedforward neural
        # networks and 2 layernorms
        rename_keys.append(
            (
...
def read_in_q_k_v(state_dict, config, base_model=False):
    for i in range(config.num_hidden_layers):
        if base_model:
            ...
            prefix = "vit."
        # read in weights + bias of input projection layer (in timm,
        # this is a single matrix + bias)
...
def convert_vit_checkpoint(vit_name):
    ...
    try:
        ...
        raise ImportError(
            "Please install the timm library to convert ViT "
            "checkpoints from timm: `pip install timm`."
        )
    ...
    if not isinstance(getattr(timm_model, "fc_norm", None), torch.nn.Identity):
        raise ValueError(
            f"{vit_name} is not supported in transformers because of "
            "the presence of fc_norm."
        )
    ...
    if "clip" in vit_name and not isinstance(
        ...
        raise ValueError(
            f"{vit_name} is not supported in transformers because "
            "it's a CLIP style ViT with norm_pre layer."
        )
    ...
    if (
        ...
        raise ValueError(
            f"{vit_name} is not supported in transformers because "
            "it's a SigLIP style ViT with attn_pool."
        )
    ...
    if not isinstance(
        ...
        raise ValueError(
            f"{vit_name} is not supported in transformers because "
            "it uses a layer scale in its blocks."
        )
    ...
    if timm_model.num_classes != 0:
        ...
        print(
            f"{vit_name} is going to be converted as a feature extractor only."
        )
        base_model = True
    ...
    # Check outputs on an image, prepared by
    # ViTImageProcessor/DeiTImageProcessor
    if "deit" in vit_name:
...
def load_model(args):
    ...
    if (
        ...
        and "timm" in args.model_name_or_path
    ):
