...
def scratchpad_bytes(node, tiled_shapes, config, extra_sharing=0):
    ...
    the caller compares the result against ``usable_scratchpad_size //
    num_slots`` rather than against the whole scratchpad.
...
def _bank_groups(node, tiled_shapes, config, extra_sharing):
    ...
    target = len(sized)
    if config.num_banks:
        target = config.usable_banks // config.num_slots
