...
those space-annotated buffers/tiles into a concrete map:
  ...
  * **DRAM** — the persistent params / inputs are placed first (no reuse), then
    ...
    greedy best-fit, shared-object allocator that reuses a slot whose lifetime is
    disjoint and whose size is closest (least fragmentation).
  * **Scratchpad** — a single fixed ``cache_size`` pool, structured as
    ``num_banks`` banks.  Each top-level compute region (a ``while_loop`` nest or
    an untiled load/compute/store group) is laid out with the existing banking
    strategies (``codegen/banking.py``), which place an op's inputs / weight /
    output in *separate banks* for crossbar bandwidth.  Regions run sequentially,
    so each resets to offset 0 and the pool size is the max region footprint.
...
reads directly.  See the roadmap in the design doc for the optimizing passes that
build on this baseline (intra-region reuse, store->load elision, double
...
def _val(node) -> Optional[torch.Tensor]:
    """The single tensor a node produces (its ShapeProp ``value`` or, inside a loop
    body, the exported ``meta['val']``); ``None`` for tuples / non-tensors."""
...
def _nbytes(node, bank_width=None) -> int:
    """Byte size of a node's tensor, using the logical (quantized) dtype when set
    and aligning to ``bank_width``."""
    t = _val(node)
    if t is None:
        return 0
...
def _greedy_best_fit(items: List[Tuple[object, int, int, int]]) -> Tuple[Dict[object, int], int]:
    """Assign each item the lowest address free of every overlapping-lifetime item.

    ``items`` is ``[(key, size, def_t, last_t), ...]``.  Process largest-first, and
    place each at the lowest offset that doesn't collide (in address) with an
    already-placed item whose lifetime overlaps — the standard "greedy by size"
    offset planner, whose high-water mark approaches the max concurrent demand (so
    sequential regions collapse onto the same low addresses, rather than each item
    getting a fresh slot).  Returns ``({key: offset}, total_bytes)``.
    """
    placed: List[Tuple[object, int, int, int, int]] = []   # (key, lo, hi, start, end)
    total = 0
    for key, size, lo, hi in sorted(items, key=lambda it: (-it[1], it[2])):
        occupied = sorted(
            (start, end) for _k, a, b, start, end in placed if a <= hi and lo <= b
        )
        off = 0
...
def _plan_dram(model: GraphModule, bank_width: Optional[int]) -> int:
    ...
    #   persistent -- inputs + weights, live the whole run (placed once, no reuse)
    #   reusable   -- alloc activation buffers, recycled once dead
    # ``buffer_of`` maps each DRAM node to the node owning its buffer.  Several nodes
    # can name the same physical buffer: an ``alloc`` owns itself, while a
    # ``getitem``/``store_tile`` is just another handle to the buffer it pulls out
    # of a loop / writes to, so it resolves back to that owner.
    ...
    for n in nodes:
        if n.op == "placeholder" and _val(n) is not None:
            ...
            if n.target is _ALLOC:
                ...
                if isinstance(src, Node) and src.target is _WHILE and isinstance(idx, int):
    ...
    # Lifetime of each buffer: from its def to the last top-level node consuming it
    # (or an alias).  A while_loop consuming a buffer keeps it live for the loop.
    ...
    # Reusable activation buffers: greedy best-fit, laid out after the persistent
    # region.
...
def _bodies(model: GraphModule, loop: Node) -> List[GraphModule]:
    """The body GraphModule of ``loop`` plus all nested loop / fused-module bodies."""
...
def _resolve_tile(n: Node, arg_map: Dict[Node, Node]) -> Optional[Node]:
    """Map a reference-op operand to the body ``load_tile`` that produces it: it is
    the operand itself for an in-body ref, or the call_module arg bound to it for a
    tail-fused submodule placeholder."""
...
def _region_roles(nodes: List[Node]):
    ...
    Returns ``(ref, output_node, key_to_node, tile_shapes)`` or ``None``.  ``ref``
    is the GEMM/conv op (preferred) or the first scratchpad-producing op;
    ``output_node`` is the node that carries its output tile (the ref, or the
    call_module that wraps it); ``key_to_node`` maps ``f"{scope}::{role}"`` to the
    ``load_tile`` for each operand; ``tile_shapes`` maps each of those tiles and the
    output node to its tile shape.
    """
    ref = next(
        (n for n in nodes
    ...
    # If ref lives inside a fused call_module, the tiles feed that module; map its
    # placeholders back to the call_module args.
    host = None
    ...
    for n in nodes:
        ...
        if sub is not None and any(x is ref for x in sub.graph.nodes):
            ...
            arg_map = {ph: a for ph, a in zip(phs, n.args) if isinstance(a, Node)}
            host = n
            break
...
def _scratch_regions(model: GraphModule) -> List[List[Node]]:
    """The compute regions whose tiles share the pad while the region runs: one per
    top-level ``while_loop`` nest, plus one per top-level untiled op (the scratchpad
    cone behind its ``store_tile``).  A cone whose only compute is a view / nop
    (reshape, flatten, ...) is DRAM-resident addressing, not a pad occupant, so it is
    dropped — otherwise it would size the pad by a whole untiled tensor."""
...
# --- global schedule + per-buffer lifetimes (scratchpad is allocated like DRAM) ---

def _walk(gm: GraphModule):
    """Yield every node in execution order, descending into a ``while_loop`` body at
    the loop's position and into a fused ``call_module`` submodule."""
...
def _timestamps(model: GraphModule) -> Dict[Node, int]:
    """A global execution timestamp per node.  A loop body lands inside its loop's
    span, so a value carried through the loop spans the whole nest."""
...
def _buffer_identity(model: GraphModule) -> _UnionFind:
    ...
    through a ``while_loop`` is one buffer — its carried operand, the body placeholder
    it binds, the body's returned value for that slot, and the loop-result
    ``getitem`` all collapse together.  So a ``zero_tile`` accumulator threaded
    through the reduction loop (zero_tile -> body arg -> accumulate-add -> getitem)
    becomes one buffer with one lifetime, instead of several co-live aliases."""
    uf = _UnionFind()
    ...
    def walk(gm: GraphModule):
        for n in gm.graph.nodes:
            ...
            if n.op == "call_function" and n.target is _WHILE:
                ...
                operands = carried + (list(n.args[3]) if len(n.args) > 3 else [])
                out = next(x for x in body.graph.nodes if x.op == "output").args[0]
                outs = list(out) if isinstance(out, (list, tuple)) else [out]
                for ph, o in zip(phs, operands):
                    if isinstance(o, Node):
                        uf.union(o, ph)
                for i, c in enumerate(carried):
                    if isinstance(c, Node) and i < len(outs) and isinstance(outs[i], Node):
                        uf.union(c, outs[i])
                walk(body)
...
def _nop_view_tiles(model: GraphModule) -> set:
    ...
    bufferized as load-whole/store-whole.  These are DRAM-resident addressing, not
    real pad occupants, so they are excluded from scratchpad allocation (otherwise a
    whole untiled tensor would size the pad)."""
    skip: set = set()
    for store in model.graph.nodes:
        if store.op == "call_function" and store.target is _STORE:
            cone = _untiled_cone(store)
...
def _buffer_lifetimes(model, uf, order, bank_width) -> Dict[Node, _Buf]:
    """Per scratchpad buffer (a union-find root): byte size, birth, and last use.
    Death follows the alias chain — a use of *any* member (e.g. the getitem of a
    carried accumulator, or the bias-add that reads it) extends the lifetime."""
...
# --- bank-aware placement: group an op's operands by bank, then best-fit by time ---
...
class _Group:
    ...
    def_t: int
    last_t: int
    members: List[Tuple[Node, int]]   # (buffer root, byte offset within the group)
    region: int


def _build_groups(region_info, strat_idx, bufs, uf, bank_size, bank_width, unroll_dim):
    """Turn each region's tiles into bank-aligned ``_Group``s under the region's
    currently-selected banking strategy: the ref op's operands are grouped by the
    bank ``evaluate`` assigns them (same bank -> packed together, different banks ->
    separate groups), and every other scratchpad buffer (accumulator / residual /
    bias) becomes its own group."""
    groups: List[_Group] = []
    placed: set = set()
    ...
    for rid, (nodes, roles) in enumerate(region_info):
        if roles is not None:
            ...
            # ``evaluate`` / ``compute_tensor_size`` read ``node.value``, which body
            # tiles lack (only ``meta['val']``); mirror it across (codegen's later
            # ShapeProp overwrites it).
            ...
            _total, node_to_seg = strategy.evaluate(
                key_to_node, output_node, tile_shapes, bank_width, bank_size, unroll_dim
            )
        ...
        # Every remaining scratchpad buffer in the region (accumulator, residual,
        # bias, or the whole region when no banking role was found) -> its own bank.
        for n in nodes:
...
def _peak_live(groups: List[_Group]) -> Tuple[int, int, List[_Group]]:
    """The timestamp of maximum concurrent scratchpad demand and the groups live
    then.  Used to decide which ops to repack when the plan overflows ``cache_size``."""
    events = []
    for g in groups:
...
def _plan_scratchpad(
    ...
    """Allocate the scratchpad like DRAM: every tile is a buffer with a lifetime, and
    buffers are packed with greedy best-fit so a tile reuses a bank whose occupant is
    already dead — across region boundaries, not reset per region.  Bank separation
    comes from each op's banking strategy (operands grouped into bank-aligned
    ``_Group``s).  If the plan exceeds ``cache_size``, the ops live at the peak are
    repacked with a more-grouped strategy (fewer banks) and the placement is redone.
    """
    bank_size = cache_size // num_banks if num_banks else None

    region_info = [(nodes, _region_roles(nodes)) for nodes in _scratch_regions(model)]
    ...
    for g in groups:
        ...
        for root, off in g.members:
            seg = Segment(
                base + off, base + off + bufs[root].size,
                MemorySpace.SCRATCHPAD, root,
            )
...
def _untiled_cone(store: Node) -> List[Node]:
    """Scratchpad nodes feeding a top-level ``store_tile`` (its load/compute group);
    the backward walk stops at DRAM operands, isolating this op's region."""
    cone: List[Node] = []
    seen = set()
    stack = [store.args[0]]
    while stack:
...
def _thread_segments(model: GraphModule) -> None:
    """Propagate ``meta['memory']`` / ``meta['scratchpad']`` from buffer roots onto
    the body placeholders bound to them and onto in-place compute tiles, so every
    ``load_tile`` source / ``store_tile`` dest / op tile resolves to an address.
    ...
    def walk(gm: GraphModule, bound: Dict[Node, Segment]):
        ...
        for node in gm.graph.nodes:
            if node.op == "placeholder":
                if node in bound and "scratchpad" not in node.meta and "memory" not in node.meta:
                    local[node] = bound[node]
                    key = "scratchpad" if bound[node].memory_space == MemorySpace.SCRATCHPAD else "memory"
                ...
                if node.target is _WHILE:
                    ...
                    if (
                        isinstance(src, Node) and src.target is _WHILE
                    ...
                    # scratchpad input (e.g. accumulate-add reuses the accumulator).
                    if node.meta.get("space") == "Scratchpad" and "scratchpad" not in node.meta:
                        for inp in node.all_input_nodes:
                            s = get(inp)
                            if s is not None and s.memory_space == MemorySpace.SCRATCHPAD:
                                _set(node, s, local)
                                break
...
def _set(node: Node, seg: Segment, local: Dict[Node, Segment]) -> None:
    local[node] = seg
    key = "scratchpad" if seg.memory_space == MemorySpace.SCRATCHPAD else "memory"
...
def _check_overlaps(arena: str, items) -> None:
    """Warn if two simultaneously-live buffers share an address range.  ``items`` is
    ``[(name, def_t, last_t, Segment), ...]`` — overlapping lifetimes must map to
    disjoint address ranges."""
    for i in range(len(items)):
        n1, a1, b1, s1 = items[i]
        for j in range(i + 1, len(items)):
            n2, a2, b2, s2 = items[j]
            if a1 <= b2 and a2 <= b1 and s1.start < s2.end and s2.start < s1.end:
                logger.warning(
                    "[MEM_OVERLAP] %s %s [%d,%d) and %s [%d,%d) overlap",
                    arena, n1, s1.start, s1.end, n2, s2.start, s2.end,
                )


def _check_invariants(model: GraphModule, bufs: Dict[Node, "_Buf"]) -> None:
    """Warn if two simultaneously-live buffers share an address range, per arena."""
    nodes = list(model.graph.nodes)
    pos = {n: i for i, n in enumerate(nodes)}
    ...
    for n in nodes:
        ...
        if seg is not None and n.meta.get("space") == "DRAM" and isinstance(_val(n), torch.Tensor):
    ...
    for root, bf in bufs.items():
        seg = next(
            (m.meta["scratchpad"] for m in bf.members if "scratchpad" in m.meta), None
        )
        if seg is not None:
...
def plan_memory(
    ...
    *,
    ...
    ``Segment``s, threading them onto loop-body tile sites; returns the pool sizes.
    Warns (consistent with the non-bufferized ``[MEM_ALLOC_FAIL]``) if the
    scratchpad plan exceeds ``cache_size``.
    """
    ...
    if cache_size is not None and scratchpad_bytes > cache_size:
        logger.warning(
            ...
            scratchpad_bytes, cache_size,
        )

    logger.info(
        "Memory plan: DRAM=%d bytes, Scratchpad=%d/%s bytes (peak region %s)",
        dram_bytes, scratchpad_bytes, cache_size, peak_region,
    )
