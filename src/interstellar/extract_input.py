...
def extract_arch_info(arch_file):
    ...
    if type(data["capacity"][0]) is list:
        capacity_list = [
            [x / num_bytes for x in data["capacity"][i]]
...
def extract_schedule_info(schedule_file, num_levels):
    ...
    schedule_hint = {}
    for loop in hint:
        schedule_hint[le.loop_table[loop]] = [
            None,
        ] * num_levels
        for level in hint[loop]:
            level_index = int(level.lstrip("level"))
            schedule_hint[le.loop_table[loop]][level_index] = [
                None,
            ] * 3
            if "order" in hint[loop][level]:
                schedule_hint[le.loop_table[loop]][level_index][0] = hint[loop][level][
                    "order"
                ]
            if "blocking_size" in hint[loop][level]:
                schedule_hint[le.loop_table[loop]][level_index][1] = hint[loop][level][
                    "blocking_size"
                ]
            if "partitioning_size" in hint[loop][level]:
                schedule_hint[le.loop_table[loop]][level_index][2] = hint[loop][level][
                    "partitioning_size"
                ]
...
def extract_info(args):
    ...
    schedule_info = (
        extract_schedule_info(args.schedule, arch_info["mem_levels"])
        if args.schedule
        else None
    )
