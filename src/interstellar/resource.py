...
class Resource(object):
    ...
    def __init__(
        ...
        array_width = [
            (
                para_count_list[i]
                if array_dim[i] == 1
                else int(math.sqrt(para_count_list[i]))
            )
            for i in range(self.num_levels)
        ]
        ...
        self.memory_partitions.append(
            [None] * 3
        )  # do not check for invalid_underutilized at last memory level
    ...
    def arch(cls, info):
        return cls(
            ...
            info["parallel_cost"],
            info["array_dim"],
            info["utilization_threshold"],
            info["replication"],
            info["memory_partitions"],
            info["invalid_underutilized"],
        )
